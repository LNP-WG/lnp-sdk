...
"""
LNP Python demo application
"""

import sys
sys.path.insert(1, '../../wrappers/python')
from lnp_node import LNPNode
from json import dumps


config = {
    'network': 'testnet',
    'datadir': '/tmp/lnp-node/'
}
...
consignment_path = '/tmp/lnp-node/output/consignment.lnp'
...
transfer_data = {
    ...
    'transaction_file': '/tmp/lnp-node/output/transaction'
}
...
asset_id = 'lnp1scxapanh6jj9ceapvxgdzr68jumjdu44ezt3ewy4h6ahz8hkd0fs6utwne'


try:
    # Start the node with config
    node = LNPNode(config['network'], config['datadir'])
