...
mkdir -pv library/src/main/java/org/lnpbp/lnp_autogen
swig -java -c++ -package "org.lnpbp.lnp_autogen" -outdir library/src/main/java/org/lnpbp/lnp_autogen swig.i

mkdir -p $JNILIBS/arm64-v8a $JNILIBS/x86_64 $JNILIBS/armeabi-v7a $JNILIBS/x86

aarch64-linux-android21-clang++ -shared swig_wrap.cxx -L$RUSTLIB/target/aarch64-linux-android/release/ -llnp -o $JNILIBS/arm64-v8a/liblnp.so -fPIC
cp -v $RUSTLIB/target/aarch64-linux-android/release/liblnp.so $JNILIBS/arm64-v8a/
cp -v $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/arm64-v8a/libc++_shared.so $JNILIBS/arm64-v8a/

x86_64-linux-android21-clang++ -shared swig_wrap.cxx -L$RUSTLIB/target/x86_64-linux-android/release/ -llnp -o $JNILIBS/x86_64/liblnp.so -fPIC
cp -v $RUSTLIB/target/x86_64-linux-android/release/liblnp.so $JNILIBS/x86_64/
cp -v $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/x86_64/libc++_shared.so $JNILIBS/x86_64/

armv7a-linux-androideabi21-clang++ -shared swig_wrap.cxx -L$RUSTLIB/target/armv7-linux-androideabi/release/ -llnp -o $JNILIBS/armeabi-v7a/liblnp.so -fPIC
cp -v $RUSTLIB/target/armv7-linux-androideabi/release/liblnp.so $JNILIBS/armeabi-v7a/
cp -v $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so $JNILIBS/armeabi-v7a/

i686-linux-android21-clang++ -shared swig_wrap.cxx -L$RUSTLIB/target/i686-linux-android/release/ -llnp -o $JNILIBS/x86/liblnp.so -fPIC
cp -v $RUSTLIB/target/i686-linux-android/release/liblnp.so $JNILIBS/x86/
